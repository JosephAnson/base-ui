...
export default function ExampleTabs() {
  ...
  React.useEffect(() => {
    ...
    renderTemplate(
      html`<tabs-root class="rounded-md border border-gray-200" .defaultValue=${'overview'}>
        <tabs-list class="relative z-0 flex gap-1 px-1 shadow-[inset_0_-1px] shadow-gray-200">
          <tabs-tab
            class="flex h-8 items-center justify-center border-0 px-2 text-sm font-normal break-keep whitespace-nowrap text-gray-600 outline-hidden select-none before:inset-x-0 before:inset-y-1 before:rounded-xs before:-outline-offset-1 before:outline-blue-800 hover:text-gray-900 focus-visible:relative focus-visible:before:absolute focus-visible:before:outline focus-visible:before:outline-2 data-[active]:text-gray-900"
            value="overview"
          >
            Overview
          </tabs-tab>
          ...
          <tabs-indicator class="absolute top-1/2 left-0 z-[-1] h-6 w-[var(--active-tab-width)] translate-x-[var(--active-tab-left)] -translate-y-1/2 rounded-xs bg-gray-100 transition-all duration-200 ease-in-out"></tabs-indicator>
        </tabs-list>
        <tabs-panel
          class="relative flex h-32 items-center justify-center -outline-offset-1 outline-blue-800 focus-visible:rounded-md focus-visible:outline-2"
          value="overview"
        >
          ${overviewIcon('size-10 text-gray-300')}
        </tabs-panel>
        <tabs-panel
          class="relative flex h-32 items-center justify-center -outline-offset-1 outline-blue-800 focus-visible:rounded-md focus-visible:outline-2"
          value="projects"
        >
          ${projectIcon('size-10 text-gray-300')}
        </tabs-panel>
        <tabs-panel
          class="relative flex h-32 items-center justify-center -outline-offset-1 outline-blue-800 focus-visible:rounded-md focus-visible:outline-2"
          value="account"
        >
          ${personIcon('size-10 text-gray-300')}
        </tabs-panel>
      </tabs-root>`,
      host,
    );
...
function overviewIcon(className?: string) {
  return svg`<svg width="40" height="40" viewBox="0 0 30 30" fill="currentcolor" class=${className ?? nothing}>
...
function projectIcon(className?: string) {
  return svg`<svg width="40" height="40" viewBox="0 0 30 30" fill="currentcolor" class=${className ?? nothing}>

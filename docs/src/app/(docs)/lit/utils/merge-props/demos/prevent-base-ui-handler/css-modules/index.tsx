    ...
    <div className={styles.Container}>
      <div className={styles.ToggleRow}>
        <div className={styles.Panel}>
          <LitFavoriteToggle locked={locked} pressed={pressed} onPressedChange={setPressed} />
        </div>
      ...
      <button
        ...
        onClick={() => setLocked((lockedState) => !lockedState)}
      >
        {locked ? 'Unlock' : 'Lock'}
      </button>
    </div>
...
interface LitFavoriteToggleProps {
  locked: boolean;
  onPressedChange: (pressed: boolean) => void;
  pressed: boolean;
}

function LitFavoriteToggle(props: LitFavoriteToggleProps) {
  const { locked, onPressedChange, pressed } = props;
  ...
  React.useEffect(() => {
    ...
    const template = Toggle({
      ...
      render: (toggleProps, state) => {
        const mergedProps = mergeProps<'button'>(toggleProps, {
          onClick(event) {
            if (locked) {
        ...
        return html`<button
          ...
          aria-label=${String(mergedProps['aria-label'] ?? '')}
          aria-pressed=${String(mergedProps['aria-pressed'] ?? false)}
          ?data-pressed=${state.pressed}
          @click=${clickHandler}
        >
          ${state.pressed ? heartFilledIcon(styles.Icon) : heartOutlineIcon(styles.Icon)}
        </button>`;
      },
    });
  ...
  }, [locked, onPressedChange, pressed]);

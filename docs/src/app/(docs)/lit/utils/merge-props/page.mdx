...
<Subtitle>A utility to merge multiple sets of Lit props.</Subtitle>
<Meta
  name="description"
  content="A utility to merge multiple sets of Lit props, handling event handlers, className, class, and style props intelligently."
/>

`mergeProps` helps you combine multiple prop objects (for example, internal props + user props) into a single set of props you can apply to an element.
It behaves like `Object.assign` (rightmost wins) with a few special cases, so common Lit patterns work as expected.

## How merging works

- For most keys (everything except `className`, `class`, `style`, and event handlers), the value from the rightmost object wins:
...
- `ref` is not merged. Only the rightmost ref is kept:
  ...
  ```
- `className` and `class` values are concatenated right-to-left (rightmost first):
...
- Event handlers are merged and executed right-to-left (rightmost first):
  ...
  - For native `Event` instances and React-style synthetic event wrappers, Base UI adds `event.preventBaseUIHandler()`. Calling it prevents Base UI's internal logic from running.
    This does not call `preventDefault()` or `stopPropagation()`.
  - For non-event values (custom events with primitive/object payloads), this mechanism isn't available and all handlers always execute.
...
```ts title="Manually chaining handlers in a function"
...
import { TypesMergeProps, TypesMergePropsN } from './types';
...
<TypesMergeProps hideDescription />
...
<TypesMergePropsN hideDescription />

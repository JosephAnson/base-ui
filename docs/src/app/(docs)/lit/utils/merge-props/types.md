...
[//]: types.ts '<-- Autogenerated By (do not edit the following markdown directly), run: pnpm docs:validate "(docs)/lit/utils/merge-props"'
...
| c?        | `InputProps<ElementType>` | -       | Props object to merge. The function will overwrite conflicting props from previous&#xA;parameters. |
| d?        | `InputProps<ElementType>` | -       | Props object to merge. The function will overwrite conflicting props from previous&#xA;parameters. |
| e?        | `InputProps<ElementType>` | -       | Props object to merge. The function will overwrite conflicting props from previous&#xA;parameters. |
...
```tsx
type ReturnValue = PropsOf<ElementType>;
```

### mergePropsN

Merges an arbitrary number of props using the same logic as [`mergeProps`](#mergeprops).

This has slightly lower performance than [`mergeProps`](#mergeprops) due to accepting an array instead of
a fixed number of arguments. Prefer [`mergeProps`](#mergeprops) when merging 5 or fewer prop sets for
better performance.

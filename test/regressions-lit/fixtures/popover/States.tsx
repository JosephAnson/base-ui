...
import { LitPopover } from '../../../../docs/src/app/(docs)/lit/components/popover/demos/shared/LitPopover';
import { LitTemplateHost } from '../../../../docs/src/app/(docs)/lit/components/popover/demos/shared/LitTemplateHost';
import baseStyles from '../../../../docs/src/app/(docs)/react/components/popover/demos/_index.module.css';

export default function PopoverStates() {
  const detachedHandle = React.useMemo(() => Popover.createHandle<string>(), []);
  ...
  const detachedTemplate = React.useCallback(
    () => html`
      ${Popover.Trigger({
        ...
        children: bellIcon('Notifications', baseStyles.Icon),
      })}
      ${Popover.Trigger({
        className: baseStyles.IconButton,
        handle: detachedHandle,
        id: 'activity',
        payload: 'activity',
        children: listIcon('Activity', baseStyles.Icon),
      })}
      ${Popover.Trigger({
        className: baseStyles.IconButton,
        handle: detachedHandle,
        id: 'profile',
        payload: 'profile',
        children: userIcon('Profile', baseStyles.Icon),
      })}
      ${Popover.Root<string>({
        handle: detachedHandle,
        children: ({ payload }) =>
              ...
              children: Popover.Popup({
                children: [
                  Popover.Arrow({
                    ...
                    children: arrowSvg({
                      arrowFillClassName: baseStyles.ArrowFill,
                      arrowInnerStrokeClassName: baseStyles.ArrowInnerStroke,
                      arrowOuterStrokeClassName: baseStyles.ArrowOuterStroke,
                    }),
                  }),
                  Popover.Title({
                  ...
                  Popover.Description({
                    ...
                    children:
                      payload === 'activity'
                        ...
                        : payload === 'profile'
                          ? 'Open the profile trigger to preview account actions.'
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gap: '12px' }}>
        <span>Closed</span>
        <LitPopover
          arrowClassName={baseStyles.Arrow}
          arrowFillClassName={baseStyles.ArrowFill}
          arrowInnerStrokeClassName={baseStyles.ArrowInnerStroke}
          arrowOuterStrokeClassName={baseStyles.ArrowOuterStroke}
          descriptionClassName={baseStyles.Description}
          iconButtonClassName={baseStyles.IconButton}
          iconClassName={baseStyles.Icon}
          popupClassName={baseStyles.Popup}
          titleClassName={baseStyles.Title}
        />
      </div>
